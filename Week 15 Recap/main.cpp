...
public:
  ...
  /* Constructor: A constructor is a member function with the same name as the
     class. It is automatically called when an object is created.

     Initialization List: The part after the colon : in the constructor is
     called the initialization list. It initializes the member variables of the class.
  ...
  void setName(WeaponType type) {
    ...
    default:
      name = "";
        break;    /* The getWeaponName function takes a WeaponType enum as an
...
private:
  ...
  int numHeals; /* The Enemy class consists of private member variables such as:
                   ...
                   weapon: an object of the Weapon class representing the enemy's weapon. 
                   numHeals: an integer storing the number of healing items the enemy has.*/
public:
  ...
  /* Enemy(EnemyType type, string name, int health, Weapon weapon): This part
     declares the constructor for the Enemy class. It takes parameters type of 
     type EnemyType, name of type string, health of type int, and weapon of type 
     Weapon. This constructor is used to create an Enemy object with the specified 
     characteristics such as type, name, initial health, and weapon.

     : name(name), health(health), weapon(weapon), numHeals(0) {}: This section
     is the initialization list of the constructor. It initializes the member 
     variables name, health, weapon with the values passed as arguments (name, 
     health, weapon) and initializes numHeals to 0.*/
    ...
    return rand() % 10 ==
           0; /* rand() Function: The rand() function is used to generate
                ...
                /* The performCriticalHit() function checks if the result of rand() % 10
                 equals 0. If the condition is met, the function returns true, indicating 
                 a critical hit. Otherwise, it returns false.*/
  }
  ...
  void heal() {
    if (numHeals < 3) {
      ...
      numHeals++; /* Conditional Check: This code checks if the number of heals
                     ...
                     post-increment operator is used, which means the current value of numHeals 
                     is used in the expression, and then numHeals is incremented by 1.
...
public:
  ...
  void useWeapon(Weapon weapon, Enemy &enemy) {
    ...
    /* The function useWeapon takes a Weapon object weapon and an Enemy object
       enemy as parameters. It uses cout to display a message indicating the attack. 
       The message includes the weapon's name obtained from getWeaponName(weapon.getType()) 
       and the damage inflicted retrieved from weapon.getDamage().

       The function calls enemy.takeDamage(weapon.getDamage()) to reduce the
       enemy's health by the damage amount.*/
  }
...
int main() {
  ...
  while (player.getHealth() > 0 && dragon.getHealth() > 0) {
    ...
    if (dragon.getHealth() > 0) {
      ...
      /* Object Health Check: This code checks the health of the dragon
      ...
      if (dragon.performCriticalHit()) {
        ...
        player.takeDamage(
            criticalDamage); /* This code checks if the dragon can perform a critical hit. 
                                If the condition is met, the dragon's weapon damage is doubled, 
                                and the player takes the increased damage.*/
        ...
        cout
            ...
            << " damage.\n"; /* This code consists of a single line where
