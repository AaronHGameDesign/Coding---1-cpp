//Aaron Holladay week 11 wednesday
#include <iostream>
#include <string>
#include <vector>
#include <algorithm>
...
public:
    ...
    enemy(item item, int enemyHealth) : heldItem(item) {
    health = enemyHealth;
}
    void attack() {
        cout << "Enemy attacks with " << heldItem.name << " causing " << heldItem.damage << " damage.\n";
    }
...
int main() {
    ...
    item enemyWeapon("Enemy Weapon", weapon);
    enemy enemyInstance(enemyWeapon, 20);    // Enemy with 20 health

    enemyInstance.attack();
    enemyInstance.takeDamage(enemyWeapon.damage);
